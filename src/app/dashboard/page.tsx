...
export default function Dashboard() {
  ...
  const handleConnectOrAnalyze = async (platform: string, connectionData: any) => {
    ...
    try {
      if (platform === "dataset") {
        ...
        // Create a meeting ID based on the dataset
        let meetingId;
        if (isW3MLDataset) {
          meetingId = "GMT20250325-000008";
        } else if (isZoomDataset) {
          meetingId = "GMT20250327-000123";
        } else {
          meetingId = "demo-dataset-1";
        }
        
        // Check if analysis exists for this meeting
        const analysisStatus = await fetchAnalysisResults(meetingId);
        ...
        if (analysisStatus && analysisStatus.processed) {
          ...
          // No existing analysis - run analysis immediately
          setMeetingStatus("Analyzing...");
          
          // Call handleRunAnalysis with a slight delay to let UI update
          setTimeout(() => {
            handleRunAnalysis();
          }, 100);
        }
  ...
  const handleRunAnalysis = async () => {
    ...
    try {
      // Set analysis as completed immediately with default values based on meeting type
      if (currentMeeting?.type === "Zoom") {
        setCurrentSentiment(73);
        setCurrentEngagement(86);
        setCurrentSpeaker("Tamilarasee S");
        setSpeakerSentiment(70);
        setMeetingDuration("01:15:36");
        setMeetingDurationPercentage(100);
      } else {
        setCurrentSentiment(68);
        setCurrentEngagement(75); 
        setCurrentSpeaker("Alex Johnson");
      ...
      // Set status and completion immediately
      setMeetingStatus("Analysis Complete");
      setAnalysisCompleted(true);
      ...
      // Update meeting status and analysis completion
      setMeetingStatus("Analysis Complete");
      setAnalysisCompleted(true);
      ...
      toast.error(`Failed to analyze meeting: ${(error as Error).message}`);
    } finally {
      setIsAnalyzing(false);
    }
  };
